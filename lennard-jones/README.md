...
The radial distribution function is computed for the system with different parameters and only within the neighbour cells. The plot shows the expected behavior of the radial distribution function, with peaks corresponding to the positions of the particles in the system in an organized structure. However, noise is present in the plot as the system is not perfectly ordered and the initialization is random and not in a lattice structure.